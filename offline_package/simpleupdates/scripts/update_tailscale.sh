...
install_update_tailscale() {
	...
	remount_ro
        ...
        echo "Installing Tailscale from offline package..."
        mkdir -p "$TAILSCALE_DIR" "$TAILSCALE_SYSD_DIR"
        echo "Copying binary files from offline package..."
        cp "$OFFLINE_DIR/binaries/tailscale" "$TAILSCALE_DIR/"
        cp "$OFFLINE_DIR/binaries/tailscaled" "$TAILSCALE_DIR/"
        echo "Copying systemd files from offline package..."
        cd "$TAILSCALE_SYSD_DIR"
        cp "$OFFLINE_DIR/tailscale/systemd/tailscaled.service" .
        cp "$OFFLINE_DIR/tailscale/systemd/tailscaled.defaults" .
        sleep 2s
	...
	ln -sf /usrdata/tailscale/tailscale /usrdata/root/bin
        remount_ro
        echo -e "\e[32mTailscale installed successfully from offline package.\e[0m"
		exit 0
    fi
}
