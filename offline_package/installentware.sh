...
# OFFLINE VERSION - Uses local packages instead of downloading
TYPE='generic'
#|---------|-----------------|
...
# Copy opkg binary and config from offline package
if [ -f "$OFFLINE_DIR/entware/opkg" ]; then
    cp "$OFFLINE_DIR/entware/opkg" /opt/bin/opkg
    chmod 755 /opt/bin/opkg
else
    echo -e '\033[31mError: opkg binary not found in offline package\033[0m'
    exit 1
fi

if [ -f "$OFFLINE_DIR/entware/opkg.conf" ]; then
    cp "$OFFLINE_DIR/entware/opkg.conf" /opt/etc/opkg.conf
else
    echo -e '\033[31mError: opkg.conf not found in offline package\033[0m'
    exit 1
fi
